...
dbdump () {
	...
	for TABLENAME in $TABLENAMES ; do
	   OUTFILENAME=`echo $2 | sed "s~$3~$3.$TABLENAME~"`
           $NICE_CMD mysqldump --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT $1 $TABLENAME > $OUTFILENAME
        done
        $NICE_CMD mysqldump --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT --no-data $1 > $2
    else
    ###
       CNT=0
       while [ $CNT -lt 2 ];do
	...
	eval $NICE_CMD mysqldump --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT $1 $COMP_CMD > $2$SUFFIX
		...
		echo "Base ($1) crashed, try $CNT repair "
                mysqlcheck --user=$USERNAME --password=$PASSWORD --host=$DBHOST -c -e --auto-repair $1
                REPAIRCODE=$?
...
dbdump_h () {
    ...
    $NICE_CMD /usr/bin/innobackupex --user=root --password=`cat /root/.mysql` --no-timestamp --rsync --tmpdir=/tmp $1 2>&1
    if [ $? -gt 0 ]; then 
      echo "Error in full backup stage" >&2
    fi
    $NICE_CMD /usr/bin/innobackupex --user=root --password=`cat /root/.mysql` --apply-log --tmpdir=/tmp $1 2>&1
    if [ $? -gt 0 ]; then 
...
dbdump_h_xtra () {
	...
	echo Full backup stage started at `date`
	#ionice -c3 
	$NICE_CMD /usr/bin/innobackupex --defaults-file=/etc/my.cnf --password=`cat /root/.mysql` --no-timestamp  --throttle=40 --rsync /var/lib/mysql-xtra 2>&1
        if [ $? -gt 0 ]; then 
          echo "Error in full backup stage" >&2
        fi
	#ionice -c3 
	$NICE_CMD /usr/bin/innobackupex --apply-log --redo-only --defaults-file=/etc/my.cnf --password=`cat /root/.mysql` --no-timestamp  --throttle=40 /var/lib/mysql-xtra 2>&1
	if [ $? -gt 0 ]; then 
    	  echo "Error in apply log redo stage" >&2
        fi
	echo Full backup stage finished at `date` code $?
	...
	#ionice -c3 
	$NICE_CMD /usr/bin/innobackupex --defaults-file=/etc/my.cnf --password=`cat /root/.mysql` --no-timestamp  --throttle=40  --rsync --incremental /var/lib/mysql-xtra-inc --incremental-basedir=/var/lib/mysql-xtra 2>&1
        if [ $? -gt 0 ]; then 
          echo "Error in inc stage" >&2
        fi
	#ionice -c3 
	$NICE_CMD /usr/bin/innobackupex --defaults-file=/etc/my.cnf --password=`cat /root/.mysql` --no-timestamp  --throttle=40 --apply-log /var/lib/mysql-xtra --incremental-dir=/var/lib/mysql-xtra-inc 2>&1
	if [ $? -gt 0 ]; then 
          echo "Error in inc apply stage" >&2
	fi
        $NICE_CMD /usr/bin/rsync -vaH --delete -f "+ */" -f "+ */**" -f "- *" /var/lib/mysql-xtra-inc/ /var/lib/mysql-xtra
        rm -rf /var/lib/mysql-xtra-inc
...
# Daily Backup
        ...
        echo
        echo Rotating last weeks Backup...
        /usr/bin/find "$BACKUPDIR/daily/" -maxdepth 1 -name "*.snapshot.sql.*" -mtime +$BACKUP_DAYS -type f -delete
        echo
